...
private:
    static SimulationOutput core_logic(const SimulationInput& in, const params& global_params, bool verbose) {
        // Create local copy of params to avoid thread conflicts
...
#pragma omp critical
        ...
        for (int i = 0; i <= f_steps; ++i) {
            ...
            for (int warmUp_step = 0; warmUp_step < warmUp_steps; ++warmUp_step) {
                ...
                V_ext_val = p.V_DC + p.V_AC * cos(angular_frequency * t_warmUp);
                RK4_step<D>(t_warmUp, current_state, p, p.dt, single_neuron_rhs<D>, V_ext_val);
            }
            ...
            if ((verbose) and (omp_get_thread_num() == 0)) {
                double percentage = static_cast<double>(i) / f_steps;

                bool is_25 = (i == f_steps / 4);
                bool is_50 = (i == f_steps / 2);
                bool is_75 = (i == (3 * f_steps) / 4);

                if (is_25 || is_50 || is_75) {
#pragma omp critical
                    {
                        std::cout << "[Thread " << omp_get_thread_num()
                            << "] Sim (V=" << p.V_DC << "): "
                            << std::round(percentage * 100) << "% complete." << std::endl;
                    }
                }
            }
        }
...
#pragma omp critical
        ...
        if (!locking_frequencies.empty()) {
            ...
            out.left_depth = sweep_results[left_depth_index].CV2;
            ...
            out.right_depth = sweep_results[right_depth_index].CV2;
...
int main() {
    ...
    default_params.num_cycles = 10.0;
    bool verbose = true;
...
// 6. Pybind11 Bindings (Uncomment for Python compilation)
/*
PYBIND11_MODULE(SelectivityExperiment, m) {
    m.doc() = "Parallel Neuron Selectivity Simulator";

    // Expose the params struct so Python can modify defaults
