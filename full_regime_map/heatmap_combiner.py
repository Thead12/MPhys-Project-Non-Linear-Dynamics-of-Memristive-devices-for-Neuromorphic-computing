...
FILE_LIST = [
    ...
    #'Master_Aggregated_Grid_interpolated.csv'
    'spike_count_up.csv',
...
def aggregate_data(files):
    ...
    v_grid = np.logspace(np.log10(all_v.min()+1e-5), np.log10(all_v.max()), 200)
    ...
    for column in aggregate_map.iloc[:, 2:]:
        ...
        mask = aggregate_map[column].notna()
    
        points = (all_v, all_r)
        values = aggregate_map[column]
        if mask.any():
            points = (all_v[mask], all_r[mask])
            values = aggregate_map[column][mask]
